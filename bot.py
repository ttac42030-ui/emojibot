...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if user.get("free_used") and not is_admin(message.from_user.id):
        await message.answer(
            "Привет! 👋 У тебя уже есть пак: "
            f"https://t.me/addemoji/{user.get('sticker_set_name')}\n\n"
            "Бесплатный эмодзи уже использован, поэтому создание ОТДЕЛЬНОГО "
            f"нового пака (с новым ником) теперь платное — {EXTRA_EMOJI_STARS_PRICE}⭐, "
            "так же как и доп. эмодзи.\n\n"
            "Если просто хочешь добавить ещё эмодзи в СВОЙ текущий пак — "
            "нажми одну из расцветок под последним сообщением о паке.\n\n"
            "Если всё равно хочешь новый пак с другим ником — напиши его ниже."
        )
    else:
        await message.answer(
...
async def create_sticker_set(bot: Bot, user_id: int, nickname: str, style_key: str) -> str:
    """Создаёт новый стикер-сет с одним эмодзи внутри и возвращает его имя.

    Вынесено в отдельную функцию, т.к. используется в двух местах:
    при бесплатном первом паке и при создании ПЛАТНОГО нового пака
    после оплаты Stars (см. successful_payment).
    """
...
async def style_chosen(call: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    # Бесплатный слот уже был использован раньше (даже если это был
    # другой ник/пак) — раньше в этом месте пак всё равно создавался
    # бесплатно, и повторный /start превращался в бесконечный источник
    # бесплатных паков. Теперь дальше без оплаты не пускаем.
    if user.get("free_used") and not is_admin(user_id):
        await call.answer()
        await bot.send_invoice(
            chat_id=user_id,
            title=f"Новый пак «{nickname}»",
            description=f"Создание набора эмодзи ({STYLE_BY_KEY[style_key].label})",
            payload=f"new_pack:{nickname}:{style_key}",
            currency="XTR",
    ...
    try:
        set_name = await create_sticker_set(bot, user_id, nickname, style_key)
...
async def successful_payment(message: Message, bot: Bot):
    ...
    if payload.startswith("new_pack:"):
        # Оплаченное создание ОТДЕЛЬНОГО нового пака (см. style_chosen).
        _, nickname, style_key = payload.split(":", 2)
        try:
            set_name = await create_sticker_set(bot, user_id, nickname, style_key)
        except Exception as exc:  # noqa: BLE001
            log.exception("Failed to create sticker set after payment")
            await message.answer(
                f"Оплата прошла, но создать пак не вышло: {exc}\nНапиши в поддержку."
            )
            return
        ...
        storage.update_user(
            ...
            nickname=nickname,
            sticker_set_name=set_name,
            emoji_count=user.get("emoji_count", 0) + 1,
        )
        await message.answer(
            f"Готово! 🎉 Новый пак: https://t.me/addemoji/{set_name}",
            reply_markup=buy_more_keyboard(),
        )
        return

    # payload вида "extra_emoji:<style_key>" — добавка в существующий пак.
